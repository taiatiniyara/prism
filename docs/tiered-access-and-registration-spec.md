...
- **Row-scope:** `SCOPE_PUBLIC` (teaser/aggregate only) · `SCOPE_OWN_UTILITY` (own rows via `USERNAME()` + the `pbiRls` user→org table — **all workflow statuses 2–5**, a *live working view*, see the status-axis note below) · `SCOPE_BENCHMARKING` (the cross-utility benchmarking family for **CEO-approved periods only** (`status_id = 5`) — no per-utility flag filter; the approval publish-gate governs what reaches this surface, see the retired-flag note below).
...
**Status axis — own-utility is live, benchmarking is approved-only (requirement, Eugene 2026-09-09):** the two surfaces differ not only by *rows* but by *workflow status*:
- **`SCOPE_OWN_UTILITY` — all active statuses (2–5).** A utility must see its **own** Utility dashboard reflect its input edits **immediately, regardless of workflow status** (Pending/Entered/Reviewed/Approved) — the operational feedback loop for the people entering data.
- **`SCOPE_BENCHMARKING` — Approved only (`status_id = 5`).** Cross-utility KPIs appear only once the utility's CEO has approved the period (`publishedPeriodCondition = reportPeriods.status_id = APPROVED_STATUS`, per `db/schema/dataEntryStatus.ts`).

⚠ **Current implementation does NOT yet meet this** — verified 2026-09-09: **every** utility-entered Power BI feed (`app/api/fact*`, and the KPI facts) is gated to `status_id = 5` via `publishedPeriodCondition`; only dimension/country-context routes are ungated. So today a utility's own numbers appear on *no* dashboard until CEO approval. Closing the gap needs a **status-aware feed split** for the own-utility surface — **#4 owns** the gold/feed shaping (an own-utility feed/partition that keeps statuses 2–5, org-scoped, distinct from the approved-only benchmarking feed) **+ refresh cadence** (import-mode refresh is periodic → edits won't show "on change" unless refresh is frequent or the own-utility feed is DirectQuery/near-real-time). **#3 owns** the compute side: for own-utility **KPIs** (not just raw inputs) to update pre-approval, the calculator must compute on **working** (unapproved) data, not only Approved. Dashboard **inputs** (`kpi_input`) update directly; KPIs depend on that compute.

**Presentation ruling (Eugene 2026-09-09): option (a) — show live KPIs with a "working/unapproved" watermark.** The own-utility dashboard shows KPIs **computed live on working data** (so a mid-entry, partial-denominator KPI *will* swing) but **visually marked as unapproved** — a "Working / Unapproved" watermark or status badge distinguishing pre-approval figures from CEO-approved ones — rather than suppressing KPIs until approval (option b). Requires the own-utility feed to carry the period's `status_id` (or a derived `is_approved` flag) so Power BI can render the indicator; the benchmarking family is unaffected (approved-only, never watermarked). #4 surfaces the status flag in the own-utility feed; the watermark/badge is a report-layout element on the Utility dashboard.
...
2. **AI / DAX (PRISM AI answers):** DAX runs under `impersonatedUserName = the requesting user's email` (same RLS), replacing the single env identity — else a Basic user's AI could surface Premium/input content the dashboard gates. DAX cache must be **entitlement-keyed**. (#4's data-service domain.)
   - **AI data-service tools obey the same tenancy scope as the token — by data class (ruling 2026-09-22, #10, prompted by #16 eval case c044).** A context-scoped **utility-provider** user (BLO/DAO/CEO/etc.) calling a tool with no explicit `utility_id` must **default to the caller's own org**, and an explicit **foreign `utility_id` must be denied** — a tool must never let a utility persona read another utility's data by omission or by asking. Split:
     - **Own-utility operational / input / workflow tools** (e.g. `getKpiStatus` / submission-status/data-entry-progress, and anything exposing KPI **inputs** or entry state) → **hard own-org scope** for context-scoped users. *(The c044 defect: `getKpiStatus({})` by a BLO returned other utilities' submission counts — a tenancy leak; fix = default to caller's org.)* **Two boundary cases pinned (Eugene affirmed 2026-09-22, "completion progress stays own-utility only"):** (a) **data-entry completion % / progress** — including the *completion facet* of `get_trend_analysis` (initially misread as benchmarking) — is data-entry status → Family A; (b) **raw fact-grain measure drill** (`drill_measure` explicit-utility path, fixed #577) is input-level → Family A. Rule of thumb: *published/approved KPIs* are the only thing that crosses utilities; *how complete/entered/raw the data is* never does.
   ...
   - **The access primitive (root-caused 2026-09-22, #4 PR #566):** the bug class is operational tools gating on **`hasBenchmarkAccess()`**, which is **true for every utility role** (BLO/CEO/EXE/MGR/DAO*) → a scoped user fell through to all-utilities. Rule: **Family-A operational tools gate on `hasGlobalUtilityAccess()` (BMO/DEV only)** — never `hasBenchmarkAccess()` — fail closed for scoped users (ignore `all_utilities=true`), default `utility_id`→caller's org, reject a foreign `utility_id`. **Family-B benchmarking tools** keep `hasBenchmarkAccess()` but stay approved-KPI-only. And the **shared scope resolver must fail CLOSED on a null org**: a non-global user with `org_id = null` resolves to *no data*, never null→all-utilities (same leak by another door). Only `hasGlobalUtilityAccess()` grants all-utilities. **Period-grain form (#4 PR #570 + follow-up):** the same split applies to the period-access predicate — Family-A operational tools use an `ownUtilityPeriodPredicate` (own-org only for non-global; **drops** the `fyReportTypePredicate()` cross-utility escape, which is a benchmarking/published concept; null-org fail-closed), while Family-B benchmarking tools keep the existing `periodAccessPredicate` (cross-utility FY reach is correct there). Don't blanket-change the shared predicate — add the own-org variant and switch only Family-A callers.
...
- **Soft (client-side embed settings):** download-charts / download-tables toggles are client-side and **bypassable** (a viewer can screenshot). So a tier's "no download" is a convenience gate, **not** an IP-hard boundary; hard extraction-prevention = RLS-hide the data (which also removes view). **DECIDED (Eugene 2026-09-09): Basic uses the SOFT gate** — disable the export UI, keep full view; Premium's paid value is convenient/bulk export, not exclusivity. Power BI has no true "view-but-no-download", and the benchmarking data is aggregate cross-utility KPIs (moderate sensitivity), so the soft gate is the pragmatic choice. **Caveat:** a download gate is only as strong as its weakest surface — the **AI/DAX path** (and any data API) must enforce the same tier (content_class + scope), or it becomes a side-door around the disabled export buttons.

**Gold/dataset prerequisites** (#4 owns the shaping):
- **`content_class` dimension** on the gold fact (`kpi_actual`→'kpi', `data_entries`→'kpi_input') — what the content roles filter. Materialising it is #4's, near-term. **⚠ Requires the governed long-fact re-model** — the current ~25 `fact*` feeds pivot measures into *column names* and drop status/category/content_class, so PBI RLS (per-**row**) can only scope by org, **not** by content_class / category / subgroup. #4's **`gold_fact_value`** (long fact = `kpi_actual` ∪ `data_entries`, carrying `owning_org_id` / `status_id` / `content_class` / `measure_category_id` / `measure_subgroup_id` as rows; DRAFT `scripts/sql/DRAFT-2026-09-09-gold-governed-fact.sql`, #397) + a PBI rebuild is the prerequisite for content_class gating, the status-axis (own-utility live), **and** §3.7 category/subgroup grant scoping. Existing wide benchmarking facts stay as-is.
...
**Report structure — DECIDED (Eugene 2026-09-09): one report + RLS.** Reuses the built per-user-identity token mechanism; split to multiple reports only if a genuine *layout* divergence emerges that RLS can't express. *(Sub-item still open: who owns the PBI workspace / service principal — an ops call, no design impact.)*
...
> **⚠ Feasibility prerequisite (from #4, 2026-09-09).** Category/subgroup-level RLS is **not free on the current Power BI model**: the ~25 `fact*` feeds pivot measures into *column names* and drop status/category/content_class, so PBI RLS (a per-**row** filter) can only scope by org — it **cannot** scope by measure category/subgroup. This capability (and the §3.6 status-axis + content_class gating) is contingent on **#4's governed long-fact re-model** (`gold_fact_value` = `kpi_actual` ∪ `data_entries`, carrying `owning_org_id` / `status_id` / `content_class` / `measure_category_id` / `measure_subgroup_id` as **rows**; DRAFT `scripts/sql/DRAFT-2026-09-09-gold-governed-fact.sql`, #397) **+ a PBI-side rebuild** of the new surfaces onto it. Conversely, #4's `gold_pbi_grant_scope` view (which flattens active grants → grantee×node for the delegated RLS role) **references the §3.7 tables above and only compiles once #10 builds them** — a mutual dependency to sequence at build time.
...
Governs **which WebApp menu items appear** for a signed-in user. This is the *menu-visibility* layer; it **composes with** two layers already specified: the plan-entitlement matrix (§0/§3.2 — *which dashboards + view/download rights within them*) and the Power BI RLS/content roles (§3.6 — *which rows/content class inside a dashboard*). A menu item shows only when this layer allows it **and** the user's plan grants ≥ view.

**Qualified baseline (Eugene 2026-09-09):**
- **Dashboards require login** — no anonymous/not-logged-in dashboard access at all.
- The consumer-facing **Dashboards** menu exposes **only the benchmarking family**: Benchmarking · Country · Sub-regional · Regional. *Which* of these a user sees, and at what depth, still follows their plan (public/default → **Teaser Samples**; paid/member → full).
- The **Utility** dashboard (which contains **KPI-inputs** data) is **never** shown to non-utility users — it is own-utility only (`SCOPE_OWN_UTILITY`, §3.6).
- **PRISM AI** and **PRISM Docs** are **privilege-gated** (not on by default): **no subscription → no AI, no Docs.** A paid **subscriber** gets AI + Docs scoped to **benchmarking KPIs only** (`content_class='kpi'`, no KPI-inputs). **Utility** staff get AI + Docs over their **own** KPIs **and** inputs (`kpi + kpi_input`, own scope).

| User class / role | Dashboards (benchmarking family) | Utility dashboard | Data Entry | PRISM AI | Docs | Payments | Settings / Subs / Seats | Profile |
|---|---|---|---|---|---|---|---|---|
...
| **Utility — non-participating** (`bm_participates=false`) | ✓ benchmarking family (per plan) | — | — | per plan | per plan | — | — | ✓ |
| **PPA_FIN** (Finance) | — | — | — | — | — | ✓ (queue) | — | ✓ |
| **BMO** (PPA admin) | ✓ all | ✓ (oversight) | ✓ (oversight) | ✓ | ✓ | ✓ (queue) | ✓ full (plans, subs, seats, users) | ✓ |
| **DEV / Developer (Innov8)** | ✓ all | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ + DEV tools | ✓ |
...
- **[RESOLVED 2026-09-09, Eugene] Download-gating hardness (§3.6)** → **soft gate** for Basic (disable export UI, keep full view; PBI has no true view-but-no-download). AI/DAX + any data API must enforce the same tier so they aren't a side-door.
- **[RESOLVED 2026-09-09, Eugene] PBI report structure (§3.6)** → **one report + RLS**. (Open ops sub-item: PBI workspace/service-principal ownership — no design impact.)
