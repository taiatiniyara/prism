...
- **2026-09-01 (#2/#3) — `measure_definitions.is_kpi` + `is_kpi_input` columns DROPPED** (PR #211, applied to `.env` DB, backup taken): Eugene-directed; #3 verified drop-safe (computed measures as KPI inputs still compute via the reactive path). `kpi_definitions.is_kpi_input` untouched. Effect: formula-builder picker now offers all active measures. Picker not a live surface yet (calculator builder DEV-gated, r10) → **no USER-IMPACT row**. *(Parked/optional, #3-owned, Eugene's call: harden standalone single-KPI "Compute now" to refresh computed-measure inputs first — pre-existing staleness, not from this change.)*
