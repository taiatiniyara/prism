...
- **Awaiting Eugene — 3:** **#2 imminent full-migration run (later today)** — Eugene's data inputs: **tariff (Standard/Lifeline) + 340/342 downtime-events** extracts, one run, **+ new-org onboarding workbook if the run adds new utilities** (step built PR #179; skip if none). ⚠ the general medallion reload is ✅ DONE (PR #107). **#2 consolidation Stage 2 — 14-legacy-values disposition** (`accounting`+`electricity` org cols hold 14 values a raw drop loses → **backup+drop vs backfill+drop**; the earlier reconciliation confirm is done — this is the new, narrower call). **FYE for 3 placeholder utilities** (Vanuatu/Pitcairn/NZ — forward pre-req, no live impact; FYE *column mechanism* landed #159/#162, values still owed). *(#10 ✅ RULED; #4 country-context ✅ DONE.)*
...
- **Awaiting Eugene (3):** **#2 imminent full-migration run today** (tariff + 340/342 + new-org workbook if new utilities) · **#2 Stage 2 14-legacy-values disposition** (backup+drop vs backfill+drop for the accounting/electricity org cols) · FYE for 3 placeholder utilities (forward). *(#10 ✅ ruled.)*
...
- **#2 utility-context/governance consolidation** (Eugene-directed, multi-stage). **Stage 1 ✅** (#241 dead tables). **#4 + #8 specs now DELIVERED** (#4: `v_utility_current_context` derived view + reader inventory [only reader = org-settings UI] + `measure_strata_history` DDL; #8: effective-dated-strata for Feeder Type + reimport detect(#2)/adjudicate(#8) contract). Per-stage gates:
>   - **Stage 2** (retire 3 org context cols → derived `v_utility_current_context`) — #11's form change ✅ merged (#252, forms no longer reference the cols). Reconciliation on p2: powerquality clean (0 values); **accounting + electricity each have 7 org-only utilities = 14 legacy values a raw drop would lose** (4 = "Undeclared" noise; VU has no report_period). ⏳ **NEW Eugene decision: backup+drop vs backfill+drop** — then one PR (remove writers + Drizzle fields + drop cols + backup), git-first.
>   - **Stage 3** (Feeder Type effective-dated strata) — ✅ **BUILT + merged** (#257: `measure_strata_history` + `effective_strata_id` resolver + `fiscal_year_for_report_period` SQL mirror + Feeder Type seeded ServiceArea@FY2026; #8-approved, grain-vs-canonical-FY fix `faaa626` locked by a 140-period parity test; Eugene confirmed boundary = FY2026 cycle). SQL apply to p2 follows (git-first); **behaviour-neutral until 2026 data.**
>   - **Stage 4** ✅ **DONE** (#251 — Fuel Supply Access removed from `factUtilityContextData`).
>   - **Org rationalisation ✅** (#247). **`bm_participates` ✅ DONE** — applied on p2 by #3 (PR #250, `874941d`): column (#233) + 44 values (20 true incl NPC / 24 false) + org-1 `is_utility`→false. Non-participant purge MOOT.
>   - *Journey-affecting: Stage 2 (settings-UI edit → data-entry) + Stage 3 (2026-boundary Feeder-Type entry screen) — audit as they land.*
...
| #2 | **consolidation Stage 2 — 14-legacy-values disposition:** `accounting`+`electricity` org cols hold 14 values (from 7 org-only utilities) a raw drop loses → **backup+drop** (keep a backup, accept loss) **vs backfill+drop** (migrate the 14 into data_entries first). 4 are "Undeclared" noise; VU has no report_period | dropping the 3 org context cols (Stage 2 completion) |
