...
> **⚠ Standing rule (2026-08-30, from #8): contested async threads anchor to `main` — repo-state outranks message-state.** When a fact flip-flops across crossed cross-session messages (e.g. measure 431's mode flipped unconditional↔derived three times in two days), **do NOT record the latest message — record what the canonical commit on `main` says, and cite the commit, not the message.** A locked/FINAL commit is the source of truth; reopen only on a new *committed* argument, never a new message.
