...
- **Awaiting Eugene (3):** **#2 imminent full-migration run today** (tariff + 340/342 extracts + new-org workbook *if* new utilities) · **#2 consolidation Stage 2 reconciliation confirm** (data_entries-wins + 7-utility backfill — org/data_entries drifted; #2 won't write on a relayed ruling) · FYE for 3 placeholder utilities (forward). *(#10 ✅ ruled 2026-08-31.)*
...
>   - **Stage 2** (retire 3 org cols `accounting_standard_id`/`electricity_regulation_id`/`powerquality_standard_id` → derived view) — ⏳ **AWAITS Eugene's DIRECT reconciliation confirm** (data_entries-wins rule + a 7-utility backfill; org vs data_entries have drifted — #2 won't write data on a relayed ruling; asked Eugene, holding).
>   - **Stage 3** (Feeder Type effective-dated strata) — specs ready; gated on implementation + §1.6 seeding (rides the reimport).
>   - **Stage 4** (drop Fuel Supply Access from utility fact) — ready.
>   - **Org rationalisation ✅** (#247 — organisations 54→44, 9 users reassigned, applied to p2). **`bm_participates` values** still owed — gated on #3's `#233` column landing. Non-participant purge is now MOOT (NPC=participant).
...
| #2 | **consolidation Stage 2 — direct reconciliation confirm:** the `data_entries`-wins rule + a 7-utility backfill (org-settings vs data_entries have drifted). #2 needs it **from Eugene directly, not relayed**, before writing data | retiring the 3 org context columns → derived `v_utility_current_context` |
