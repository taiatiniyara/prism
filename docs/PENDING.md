...
- **2026-09-22 (#16) — `ai_chat_turn` +2 cache-token cols** (PR #536 `c5ebcec`, SQL-only, applied to p2): additive nullable ints `token_count_cache_read` / `token_count_cache_write` (prompt-cache telemetry for the AI optimiser). Internal telemetry → **no USER-IMPACT row**. Paired Drizzle model + code PR (1b) about to open.
