...
- **Still open / awaiting:** the **340/342 downtime measures = 0** (extract omission, needs a data slice) · the **Tier-2 structural campaign** (period dim #524 landed → kpi_target #534 open → kpi_actual roll-up …; Eugene greenlights each step, #8 gives semantics sign-offs) · re-verify the 2026-09-03 **VPS disk/health** flag (status unknown after the gap).
...
- **Awaiting Eugene (3):** **#2 imminent full-migration run today** (tariff + 340/342 + new-org workbook if new utilities) · FYE for 3 placeholder utilities (forward) · **VPS disk/health check** (infra risk — 2 npm corruption deploy-failures in ~1h; deploys self-heal now but root cause unchecked; #1 flagged). *(#2 consolidation ✅ COMPLETE — Stage 2 done #266.)*
...
- **2026-09-22 (#16) — `ai_chat_turn` +2 cache-token cols** (PR #536 `c5ebcec`, SQL-only, applied to p2): additive nullable ints `token_count_cache_read` / `token_count_cache_write` (prompt-cache telemetry for the AI optimiser). Internal telemetry → **no USER-IMPACT row**. Code pairs #537/#540 ✅ merged+deployed → #16's PR 1/2 complete, nothing pending. (Not-started #16 follow-ups: AI answer-quality eval set; report-output design + model choice — future, not tracked until begun.)
...
| Eugene (infra) | **VPS disk/health check** — TWO npm corruption deploy-failures in ~1h on 2026-09-03 (ENOTEMPTY, then ENOENT) suggest an underlying VPS disk/health issue. Deploys **self-heal now** (retry + `npm cache clean --force`, #269/#297), so not blocking — but the root cause is unchecked. #1 flagged; a `df -h` / VPS-health look is worth it. | nothing today (self-healing); pre-empts future deploy outages |
