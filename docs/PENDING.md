...
- **#2 utility-context/governance consolidation** (Eugene-directed). ✅ **Done:** Stage 1 (#241), Stage 3 (#257 `d74e174` — applied+verified on p2, behaviour-neutral until 2026), Stage 4 (#251), org rationalisation (#247), bm_participates (#3 #250). **Only Stage 2 left** — retire the 3 org context cols → derived `v_utility_current_context` (#11 forms merged #252). ⏳ **Eugene: backup+drop vs backfill+drop** for the 14 legacy values in `accounting`+`electricity` (7 org-only utilities; 4 = "Undeclared" noise; VU has no report_period). Then one git-first PR (remove writers + Drizzle fields + drop cols + backup). *Journey-affecting: Stage 2 (settings-UI → data-entry) + Stage 3 (2026 Feeder-Type entry screen — audits when 2026 data lands).*
