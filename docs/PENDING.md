...
**2 human PRs open:**
- `#104` **docs(data-availability) §3.1.1 — obligation is dimension-aware** — ⚠ **Eugene's own PR** (authored 2026-08-16 mid-outage; its "recirculate for #8/#3/#4 comment" ask was a lost delivery). **Substantive** amendment to the ratified availability design (obligation moves to slice grain). ✅ **FULLY REVIEWED 2026-08-31 — #8 ✅ · #3 ✅ · #4 ✅**, all endorsed (#4 confirmed `measure_dimension_applicability` is the correct home + a nullable `is_mandatory`-override / COALESCE-fallback shape rec, consistent with measure-relevance-spec §10's three-axis split). **Review debt cleared → ready for Eugene's merge whenever he chooses.**
...
**Net pending right now:** **2 human PRs** — `#104` (Eugene's — ✅ fully reviewed #8/#3/#4, ready for his merge) + `#77` (parked by design); 12 Dependabot (vuln triage ✅ COMPLETE — accepted-residual, nothing prod-reachable); *(review debt on #104 CLEARED 2026-08-31 — all three endorsed);* Eugene decisions (#2 extract, #10 plan/entitlements, 3 null `financial_year_end`); **#2 context-fed pass** (parts 3–4: loader guard + medallion §1.5 rule; non-blocking); #12's P2/D2 operator steps (env/Power BI/rotate); **#11 follow-up** — country-context form (metric select + upsert + not-available option, USER-IMPACT r16); **prod apply debt** — see the 🚀 **Cutover Runbook** (now ~13 steps: SQL incl. `kpi_actual` + downtime reactivate, seed, `formula_binding` tables, `verify-relevance` gate — all live on dev, per-env owed at prod migration); **forward requirement** (#4, ✅ captured in the relevance spec + #2's `reconcilePeriod`): utility-facing "shells requested" count excludes `is_calculated=true`; **deferred: unified relevance surface** — the service-area-capability SPAN spec was **superseded by `docs/measure-relevance-spec.md`** (unified relevance surface); its DDL rides **#2's ONE combined temporal-spans reimport migration** with unit-activation stints (`btree_gist`) — one cutover step, nothing to queue today; **relevance cluster** (#8, `187be5f`+`db339b1`) — ✅ **FULLY SEALED 2026-08-28**: polarity/gen-scope/transmission-keying/unconditional-handling ruled; tariff stays measure-level conditional (member-level gate only for mixed always+gated members — transmission the sole such case); measure 431 = **DERIVED (`conditional_default_off`), 16-off / 4-on / rest** — ✅ **EUGENE RULED synchronously 2026-08-30** (`e92f75d` / #153; spec §431-note). The note: value "was set by Eugene's direct synchronous ruling after the flag oscillated nine times on crossed async messages… not a consensus artifact and exempt from all anchor/HEAD-alignment mechanics. **No session commits any further change to this value for any reason.**" **TERMINAL — #15 ignores all further 431 pings** (stale post-ruling ones, incl. a `62a347d` "unconditional", were correctly NOT applied). Zero migration impact. Ready for the joint spans+stints+relevance pass at package assembly; a branch-triage pass over the ~15 un-PR'd `claude/*` branches. `npm install` after pull.
...
> **Forward (not queued yet):** #2's **ONE combined temporal-spans reimport migration** will carry both the `service_area_capabilities` span table (#4, ratified `2f5c357`) **and** unit-activation stints (`btree_gist`) + their seed-spans — expect a *single* cutover step for both when that migration is authored, not two.
...
- ✅ **Covered:** MFA (r1), "Grid" label (r2), sentinel deletion (r3), API-key tiering (r4), BSC "+Add KPI" picker (r8), trajectory removal (r9), Description drop (r11), is_aggregated drop (r12), external_registrations (r13), vocab rename (r14), managed-lists deletion (r15), FY-aware ReportPeriod labels (r17, #4 `6504e7e`), **data-entry validation live + financial sign/range conventions (r18, #3 PR #150/#155)**. *(Ledger now 18 rows; no gaps.)*
...
| — | **merge #104** (Eugene's own PR) — ✅ fully reviewed (#8/#3/#4 all endorsed 2026-08-31); merge whenever | closes the human-PR backlog |
