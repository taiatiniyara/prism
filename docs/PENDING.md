...
- **#2 utility-context/governance consolidation** (Eugene-directed, multi-stage; design converged #4+#8). **Stage 1 ✅** (#241 — dead `utility_context_data`+`governance_data` tables + orphaned pages retired). Remaining: single-home utility-context+governance in `data_entries`; retire 3 duplicating org cols (`accounting_standard_id`/`electricity_regulation_id`/`powerquality_standard_id` → derived); Feeder Type → service_area grain (2026+ only); drop Fuel Supply Access from utility fact. **Gated on deliverables owed: #4 derived-helper spec + #8 Feeder-Type grain spec** (both requested). *(Later stages likely journey-affecting — will audit as they land.)*
