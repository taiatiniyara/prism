...
**Net pending right now:** **2 human PRs** — `#104` (Eugene's, needs #3/#4 review) + `#77` (parked by design); 12 Dependabot (vuln triage ✅ COMPLETE — accepted-residual, nothing prod-reachable); **review debt: #4 owes a comment on #104** (#8 + #3 endorsed); Eugene decisions (#2 extract, #10 plan/entitlements, 3 null `financial_year_end`); **#2 context-fed pass** (parts 3–4: loader guard + medallion §1.5 rule; non-blocking); #12's P2/D2 operator steps (env/Power BI/rotate); **#11 follow-up** — country-context form (metric select + upsert + not-available option, USER-IMPACT r16); **prod apply debt** — see the 🚀 **Cutover Runbook** (10 idempotent SQL + seed + `verify-relevance` gate, all live on dev, per-env owed at prod migration); **forward requirement** (#4, ✅ now captured in the capability spec + #2's `reconcilePeriod` refinement): utility-facing "shells requested" count excludes `is_calculated=true`; **deferred spec — RATIFIED-by-direction** (`service-area-capability-spec.md`, `1c10a92`→`2f5c357`; span model + provenance cols): its DDL (`service_area_capabilities` span table) rides **#2's ONE combined temporal-spans reimport migration** with unit-activation stints (`btree_gist`) — one cutover step for both span tables + seed-spans, not two; nothing to queue today; a branch-triage pass over the ~15 un-PR'd `claude/*` branches. `npm install` after pull.
