...
- **Open PRs: 21** (full re-scan 2026-08-05) — **5 human** (`#107` measures/status-enum, `#104` data-availability, `#83` #12 USER-IMPACT rows, `#77` #11 access-plans proto, `#76` #12 RLS-D1) + 16 Dependabot. *(Earlier "10, all Dependabot" was stale — see §1 correction.)*
...
### Open PRs (21 total — full re-scan 2026-08-05)
> ⚠ **Correction:** earlier refreshes said "10 open, all Dependabot." That drifted stale — the open-PR/branch view depends on scanning, not pings, and wasn't re-scanned between refreshes. True state below.

**5 human PRs (awaiting review/merge):**
- `#107` [#? measures] **Measures catalogue regen + data-entry status enum enrichment** — the branch the shared main tree is currently checked out on.
...
- `#76` [#12/D1] **align RLS design to the `data_entries` ruling + no-sentinel rule** — this is the D1 RLS item (was "#12 to write policy DDL"; now a PR).

**16 Dependabot PRs (Eugene reviews individually; several superseded older ones):** `#108` dev-deps group · `#101` js-yaml · `#97` ip-address · `#93` fast-uri · `#91` undici · `#89` @types/node 26.1.2 · `#88` jsdom 30 · `#87` read-excel-file 9.3.5 · `#86` lucide-react 1.28 · `#85` @ai-sdk/anthropic 4.0.25 · `#48` eslint-plugin-security · `#47` **typescript 7** · `#46` jest-dom 7 · `#44` eslint 10 · `#39` codeql-action 4 · `#38` actions/checkout 7.
