...
- **Awaiting Eugene (2):** **#2 imminent full-migration run today** (tariff + 340/342 extracts + new-org workbook *if* new utilities) · FYE for 3 placeholder utilities (forward). *(#10 ✅ ruled 2026-08-31; consolidation Stage 2 reconciliation ✅ confirmed — now impl-ready.)*
...
>   - **Stage 2** (retire 3 org cols `accounting_standard_id`/`electricity_regulation_id`/`powerquality_standard_id` → derived `v_utility_current_context` view) — ✅ **reconciliation CONFIRMED by Eugene** (data_entries wins on the drift; the 7-utility backfill is **moot** — non-participants shouldn't carry data_entries). Now **implementation-ready** — sequence: **#11 removes the 3 org-form fields**, then **#2** adds the view + removes the writer + drops the cols. No longer Awaiting-Eugene.
>   - **Stage 3** (Feeder Type effective-dated strata) — implementation-ready (#8 supplied integration sites); + §1.6 seeding rides the reimport.
