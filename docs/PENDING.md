...
12. **`formula_binding` + `formula_binding_dimension`** tables (#3, PR #135, applied to dev) — durable 10-dim binding store for the unified formula builder. Additive. ⚠ **prod-apply path TBD** (no standalone script named — confirm with #3 whether it's a migration file or `drizzle-kit push`).
