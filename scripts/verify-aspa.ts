...
async function main() {
  // Find ASPA
  const aspa = await db.select({ id: organisations.id, acronym: organisations.acronym, name: organisations.name })
    .from(organisations).where(eq(organisations.acronym, "ASPA")).limit(1);
  if (!aspa[0]) { console.log("ASPA not found"); process.exit(1); }
  ...
  const rps = await db.select({ id: reportPeriods.id, date: reportPeriods.report_date, type: reportPeriods.report_type_id })
    .from(reportPeriods).where(eq(reportPeriods.utility_id, utilId)).orderBy(reportPeriods.report_date);
  ...
  const sas = await db
    ...
    .from(serviceAreas)
    .where(and(eq(serviceAreas.is_active, true), eq(serviceAreas.is_virtual, false), eq(serviceAreas.utility_id, utilId)));
  const saIds = sas.map(s => s.id);

  const allErs = await db
    .select({ id: energyResources.id, utility_id: energyResources.utility_id, energy_source_id: energyResources.energy_source_id, period_entries: energyResources.period_entries })
    .from(energyResources);

  const existingEntries = await db
    .select()
    .from(dataEntries)
    .where(and(eq(dataEntries.is_deleted, false), eq(dataEntries.is_relevant, true), inArray(dataEntries.report_period_id, rpIds)));

  const irrelevantDE = await db
    .select({ reportPeriodId: dataEntries.report_period_id, inputDefId: dataEntries.input_def_id, serviceAreaId: dataEntries.service_area_id })
    .from(dataEntries)
    .where(and(eq(dataEntries.is_deleted, false), eq(dataEntries.is_relevant, false), inArray(dataEntries.report_period_id, rpIds)));
  ...
  const irrelevantInputRel = await db
    ...
    .from(inputRelevance)
    .where(and(eq(inputRelevance.is_relevant, false), inArray(inputRelevance.input_def_id, genIds.length > 0 ? genIds : [-1])));
  const periodInputRelIrrelevant = new Set(irrelevantInputRel.map(r => `${r.inputDefId}:${r.dimensionId}`));
  ...
  console.log(`  ${existingEntries.length} relevant entries across ${rpIds.length} periods`);
  console.log(`  ${irrelevantDE.length} irrelevant entries`);
  console.log(`  ${irrelevantInputRel.length} irrelevant input_relevance rows\n`);

  for (const rp of rps) {
    const rpId = rp.id;
    const entriesForPeriod = existingEntries.filter(e => e.report_period_id === rpId);
    ...
    irrelevantDE.filter(r => r.reportPeriodId === rpId).forEach(r => {
    ...
    for (const e of entriesForPeriod) {
      if (!genIds.includes(e.input_def_id)) {
        actualNonGenSACombos.add(`${e.input_def_id}:${String(e.service_area_id)}`);
      }
    }
    ...
    for (const er of periodErs) {
      for (const inputDefId of genIds) {
        if (periodInputRelIrrelevant.has(`${inputDefId}:${er.energy_source_id}`)) continue;
        genExpected++;
      }
    }
    ...
    console.log(`RP ${rpId} (${rp.date.toISOString().split('T')[0]}): Req=${requested} Pnd=${pending} Ent=${entered} Rev=${reviewed} App=${approved} End=${endorsed} NA=${na} Compl=${completed} (${((completed/requested)*100).toFixed(0)}%) ng=${nonGenExpected} g=${genExpected} ERs=${periodErs.length}`);
  }
