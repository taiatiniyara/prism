...
async function main() {
  ...
  const er2 = await db.select({ id: energyResources.id, name: energyResources.name, period_entries: energyResources.period_entries }).from(energyResources).where(sql`id = 255`).limit(1);
  
  if (er1[0]) {
    const pe1 = er1[0].period_entries as any[];
  ...
  for (const er of util16Ers) {
    ...
    const activeCount = pe.filter((e: any) => e.is_active).length;
    const uniqueRpIds = new Set(pe.map((e: any) => e.report_period_id ?? e.utility_report_period_id));
  ...
  console.log(`\nTotal report periods for Nauru: ${(rpCount as any).rows[0].count}`);
