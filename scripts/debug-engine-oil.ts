...
async function main() {
  const [scope] = (
    await pool.query(`
      ...
      join data_entries d
       ...
       and d.measure_def_id = 1659 and d.is_deleted = false
       and d.value is not null and trim(d.value) <> ''
      join report_periods rp on rp.id = n.report_period_id
      where n.measure_def_id = 1652 and n.is_deleted = false
  ...
  const rows = await pool.query(
    `select measure_def_id, value, is_relevant, is_deleted, service_area_id,
            energy_resource_id, energy_provider_id, energy_source_id,
            customer_type_id, payment_mode_id
     from data_entries
     where measure_def_id in (1652, 1659)
