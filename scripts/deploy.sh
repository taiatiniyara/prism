...
VPS_PATH="${VPS_PATH:-/home/deploy/prism}"
...
ssh "${VPS_USER}@${VPS_HOST}" "
  set -e
  mkdir -p ${BACKUP_DIR}
  PGPASSWORD=\$(grep DATABASE_URL ${VPS_PATH}/.env | cut -d'/' -f3 | cut -d'@' -f1 | cut -d':' -f3) || true
  DB_NAME=\$(grep DATABASE_URL ${VPS_PATH}/.env | grep -oP '[^/]+(?=\?)' | head -1)
  DB_USER=\$(grep DATABASE_URL ${VPS_PATH}/.env | grep -oP '//\K[^:]+' | head -1)
  DB_HOST=\$(grep DATABASE_URL ${VPS_PATH}/.env | grep -oP '@\K[^:]+' | head -1)
  if command -v pg_dump &>/dev/null && [ -n \"\$DB_NAME\" ]; then
    pg_dump -h \"\$DB_HOST\" -U \"\$DB_USER\" -d \"\$DB_NAME\" -F c -f \"${BACKUP_FILE}\" 2>/dev/null || \
      ...
      { echo \"ERROR: pg_dump failed, aborting deployment\"; exit 1; }
    echo \"Backup saved to ${BACKUP_DIR}/${BACKUP_FILE}\"
  else
    echo \"ERROR: pg_dump not available, aborting deployment\"
    exit 1
  fi
"
