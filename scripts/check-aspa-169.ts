...
async function main() {
  ...
  const entries = await db
    ...
    .where(
      and(
        ...
        inArray(dataEntries.measure_def_id, genIds.length > 0 ? genIds : [-1]),
      ),
    );
  ...
  const combos = new Set(
    entries.map((e) => `${e.inputDefId}:${e.energyResourceId}`),
  );
  ...
  const nonGenGenIds = nonGenEntries.filter((e) =>
    genIds.includes(e.inputDefId),
  );
  const nonGenNonGenIds = nonGenEntries.filter(
    (e) => !genIds.includes(e.inputDefId),
  );
  ...
  const orphanEntries = nonGenEntries.filter(
    (e) => !allActiveIds.has(e.inputDefId),
  );
  console.log(
  ...
  for (const e of orphanEntries.slice(0, 5)) {
    console.log(
      `  def=${e.inputDefId} sa=${e.serviceAreaId} status=${e.statusId}`,
    );
  }

  // Status breakdown
  const byStatus = nonGenEntries.reduce(
    (acc, e) => {
