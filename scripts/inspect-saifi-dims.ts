...
async function main() {
  const tags = await pool.query(`
    select de.input_def_id, de.energy_provider_id, de.energy_source_id,
           src.parent_id as derived_type_id, count(*)::int as rows
    from data_entries de
    left join managed_list_items src on src.id = de.energy_source_id
    where de.input_def_id in (153, 1800, 1803, 1652, 1659)
      and de.is_deleted = false and de.value is not null and trim(de.value) <> ''
    group by 1, 2, 3, 4
    order by de.input_def_id, rows desc
  `);
  console.log("Distinct dimension tags per input (provider, source, derived type):");
  ...
  `);
  console.log("Events scopes where customers exist on the SAME (period, SA, resource):");
