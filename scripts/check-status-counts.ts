...
async function main() {
  ...
  for (const s of byStatus) {
    console.log(
      `  ${labels[s.status_id] ?? `?(${s.status_id})`}: ${s.cnt.toLocaleString()}`,
    );
  }
  ...
  const withValue = await db
    ...
    .from(dataEntries)
    .where(
      sql`${dataEntries.is_deleted} = false AND ${dataEntries.value} IS NOT NULL AND ${dataEntries.value} != ''`,
    );
  const withoutValue = await db
    .select({ cnt: count() })
    .from(dataEntries)
    .where(
      sql`${dataEntries.is_deleted} = false AND (${dataEntries.value} IS NULL OR ${dataEntries.value} = '')`,
    );
  ...
  for (const r of rpCounts) {
    console.log(
      `  RP ${r.rpId}: total=${r.total} entered=${r.entered} reviewed=${r.reviewed} approved=${r.approved} na=${r.notAvail} pending=${r.pending} withVal=${r.withVal}`,
    );
  }
  ...
  const pendingWithValue = await db
    ...
    .from(dataEntries)
    .where(
      sql`${dataEntries.status_id} = 2 AND ${dataEntries.value} IS NOT NULL AND ${dataEntries.value} != '' AND ${dataEntries.is_deleted} = false`,
    );
  console.log(
  ...
  const enteredNoValue = await db
    ...
    .from(dataEntries)
    .where(
      sql`${dataEntries.status_id} = 3 AND (${dataEntries.value} IS NULL OR ${dataEntries.value} = '') AND ${dataEntries.is_deleted} = false`,
    );
  console.log(
    `  Entered (status 3) entries WITHOUT values: ${enteredNoValue[0].cnt}`,
  );
  ...
  const requested = await db
    ...
    .from(dataEntries)
    .where(
      sql`${dataEntries.status_id} = 1 AND ${dataEntries.is_deleted} = false`,
    );
