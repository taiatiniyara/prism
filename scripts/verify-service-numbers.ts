...
async function main() {
  ...
  const rpId = 193;
  const tauId = 24;
  const tau = await db.select({ acronym: organisations.acronym }).from(organisations).where(eq(organisations.id, tauId)).limit(1);
  ...
  const cats = new Set(definitionRows.map(r => r.categoryName?.trim().toLowerCase()).filter(Boolean));
  const subcats = new Set(definitionRows.map(r => r.subcategoryName?.trim().toLowerCase()).filter(Boolean));
  ...
  const sas = await db
    ...
    .from(serviceAreas)
    .where(and(eq(serviceAreas.is_active, true), eq(serviceAreas.is_virtual, false), eq(serviceAreas.utility_id, tauId)));
  ...
  for (const er of periodErs.slice(0,5)) console.log(`  ER ${er.id} es=${er.energy_source_id}`);
  ...
  irrelevantDE.filter(r => r.reportPeriodId === rpId).forEach(r => {
    const existing = periodIrrelevantDE.get(r.serviceAreaId) ?? new Set<number>();
  ...
  irrelevantInputRel.forEach(r => periodInputRelIrrelevant.add(`${r.inputDefId}:${r.dimensionId}`));
  ...
  console.log(`Progress:                ${((completed / requested) * 100).toFixed(0)}% (${completed}/${requested})`);
