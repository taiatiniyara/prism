...
import { dataEntries, inputDefinitions, type FormulaInput } from "@/db/schema/dataEntry";
...
async function main() {
  // Check ASPA
  const aspa = (await db.select({ id: organisations.id, acronym: organisations.acronym }).from(organisations).where(eq(organisations.acronym, "ASPA")).limit(1))[0];
  if (!aspa) { console.log("ASPA not found"); process.exit(1); }
  console.log(`=== ${aspa.acronym} id=${aspa.id} ===\n`);

  const rps = await db.select({ id: reportPeriods.id, date: reportPeriods.report_date }).from(reportPeriods).where(eq(reportPeriods.utility_id, aspa.id)).orderBy(reportPeriods.report_date);
  ...
  const sas = await db.select({ id: serviceAreas.id }).from(serviceAreas).where(and(eq(serviceAreas.is_active, true), eq(serviceAreas.is_virtual, false), eq(serviceAreas.utility_id, aspa.id)));
  const saIds = sas.map(s => s.id);
  console.log(`SAs: ${saIds.length} → [${saIds.join(',')}]`);

  const allErs = await db.select({
    id: energyResources.id, utility_id: energyResources.utility_id,
  ...
  const allEntries = await db.select().from(dataEntries).where(and(eq(dataEntries.is_deleted, false), eq(dataEntries.is_relevant, true), inArray(dataEntries.report_period_id, rps.map(r => r.id))));

  const irrelDE = await db.select({ rpId: dataEntries.report_period_id, inputDefId: dataEntries.input_def_id, saId: dataEntries.service_area_id }).from(dataEntries).where(and(eq(dataEntries.is_deleted, false), eq(dataEntries.is_relevant, false), inArray(dataEntries.report_period_id, rps.map(r => r.id))));
  ...
  for (const rp of rps) {
    ...
    for (const e of periodEntries) {
      if (e.status_id === 3) entered++; else if (e.status_id === 4) reviewed++;
      else if (e.status_id === 5) approved++; else if (e.status_id === 6) endorsed++;
      else if (e.status_id === 7) na++;
    }

    // Irrelevant map
    const irrelBySA = new Map<string, Set<number>>();
    for (const r of irrelDE) { if (r.rpId !== rpId) continue;
      const k = String(r.saId ?? "null"); const s = irrelBySA.get(k) ?? new Set<number>(); s.add(r.inputDefId); irrelBySA.set(k, s);
    }
    ...
    for (const def of defRows) {
      ...
      const formulaInputs = (def.formulaInputs as FormulaInput[] | undefined) ?? [];
        ...
        if (epId != null || esId != null) {
          ...
          for (const gen of matchingGens) {
            if (gen.service_area_id && saIds.includes(gen.service_area_id)) {
              const irrelSA = irrelBySA.get(String(gen.service_area_id)) ?? new Set<number>();
    ...
    console.log(`RP ${rpId} (${rp.date.toISOString().split('T')[0]}): Req=${requested} Pnd=${pending} Ent=${entered} Rev=${reviewed} End=${endorsed} Compl=${completed} (${((completed/requested)*100).toFixed(0)}%) gens=${periodGens.length}`);
  }
