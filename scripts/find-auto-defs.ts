...
async function main() {
  ...
  const autoMapped = await db
    .select({ defId: inputDlDefMappings.input_def_id, trainingId: inputDlDefMappings.training_dl_def_id, trainingName: inputDlDefMappings.training_dl_name })
  ...
  const newDefs: Array<{ defId: number; trainingId: number; trainingName: string }> = [];
  
  for (const m of autoMapped) {
    const def = await db.select({ id: inputDefinitions.id, name: inputDefinitions.name, sort_order: inputDefinitions.sort_order })
      .from(inputDefinitions)
      .where(eq(inputDefinitions.id, m.defId))
      .limit(1);
    ...
    if (prismName === trainingName) {
      newDefs.push({ defId: m.defId, trainingId: m.trainingId, trainingName: m.trainingName });
    }
  }

  console.log(`Auto-created defs (new, not matched): ${newDefs.length}`);
  for (const d of newDefs) {
    console.log(`  prism:${d.defId} ← training:${d.trainingId} "${d.trainingName}"`);
  }
  ...
  if (newDefs.length > 0) {
    ...
    const entryCount = await db.execute(sql`
      SELECT count(*) FROM data_entries WHERE input_def_id IN (${sql.join(ids.map(String), sql`,`)}) AND is_deleted = false
    `);
    ...
    for (const d of newDefs) {
      ...
      const similar = await db.execute(sql`
        SELECT id, name FROM input_definitions 
