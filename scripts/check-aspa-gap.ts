...
async function main() {
  ...
  for (const e of entries) {
    if (genIds.has(e.measure_def_id)) {
      // Gen entry
      if (e.energy_resource_id && !activeErIds.has(e.energy_resource_id)) {
        gap1++;
        if (gap1 <= 3)
          console.log(
            `  Gap: gen def=${e.measure_def_id} ER=${e.energy_resource_id} (ER not active for this period)`,
          );
      }
    } else {
      // Non-gen entry
      if (!defSet.has(e.measure_def_id)) {
        gap2++;
        if (gap2 <= 3)
          console.log(
            `  Gap: non-gen def=${e.measure_def_id} (def not in definitionRows)`,
          );
      }
      if (e.service_area_id && !saIds.has(e.service_area_id)) {
        ...
        if (gap3 <= 3)
          console.log(
            `  Gap: non-gen def=${e.measure_def_id} SA=${e.service_area_id} (SA not in utility SA list)`,
          );
      }
    }
  }
