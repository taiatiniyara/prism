...
async function main() {
  ...
  const data = await pool.query(`
    select de.input_def_id, de.is_deleted,
           count(*)::int as rows,
    ...
    where de.input_def_id in (1802, 1805, 1808)
  ...
  const overlap = await pool.query(`
    with dur as (
      select report_period_id, service_area_id, energy_resource_id, input_def_id
      from data_entries
      where input_def_id in (1802, 1805) and is_deleted = false
    ...
    group by dur.input_def_id
  `);
  console.log("Duration scopes with customers-served available (SAIDI feasibility):");
