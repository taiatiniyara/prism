...
import { inputDefinitions, inputDlDefMappings, dataEntries, dataEntryLogs } from "@/db/schema/dataEntry";
...
async function main() {
  ...
  for (const def of allDefs) {
    ...
    const original = allDefs.find(d => d.name === def.name && !autoDefIds.has(d.id));
    if (original) {
  ...
  console.log(`Genuinely new defs to keep: ${toKeep.size - (allDefs.length - autoDefIds.size)}`);
  ...
  for (const [dupId, origId] of duplicates) {
    ...
    const entryCount = await db
      ...
      .from(dataEntries)
      .where(eq(dataEntries.input_def_id, dupId));
    
    if (entryCount[0].cnt > 0) {
      // Remap data entries
      await db.update(dataEntries)
        .set({ input_def_id: origId })
        .where(eq(dataEntries.input_def_id, dupId));
    }

    // Update mappings
    await db.update(inputDlDefMappings)
      .set({ input_def_id: origId })
      .where(eq(inputDlDefMappings.input_def_id, dupId));

    remapped++;
    if (remapped % 50 === 0) console.log(`  Remapped ${remapped}/${duplicates.size}...`);
  }
  ...
  if (duplicates.size > 0) {
    ...
    await db.delete(inputDlDefMappings)
      .where(and(
        inArray(inputDlDefMappings.input_def_id, dupIds),
        // Only if NOT remapped to original (safety)
      ));
    
    // Delete from input_definitions
    await db.delete(inputDefinitions)
      .where(inArray(inputDefinitions.id, dupIds));
    
    console.log(`Deleted ${dupIds.length} duplicate defs`);
  }

  // Verify
  const remaining = await db.select({ cnt: sql<number>`count(*)` }).from(inputDefinitions);
  ...
  const orphanEntries = await db
    .select({ cnt: sql<number>`count(*)` })
    .from(dataEntries)
    .where(notInArray(dataEntries.input_def_id, 
      (await db.select({ id: inputDefinitions.id }).from(inputDefinitions)).map(d => d.id)
    ));
