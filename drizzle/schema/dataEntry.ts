...
export const inputDefinitions = pgTable('input_definitions', {
    ...
    category_id: integer('category_id').notNull().references(() => managedListItems.id),
    subcategory_id: integer('subcategory_id').notNull().references(() => managedListItems.id),
    energy_provider_id: integer('energy_provider_id').references(() => managedListItems.id),
    energy_type_id: integer('energy_type_id').references(() => managedListItems.id),
    energy_source_id: integer('energy_source_id').references(() => managedListItems.id),
    unit_id: integer('unit_id').notNull().references(() => managedListItems.id),
    data_type_id: integer('data_type_id').notNull().references(() => managedListItems.id),
...
export const dataEntries = pgTable('data_entries', {
    ...
    input_def_id: integer('input_def_id').notNull().references(() => inputDefinitions.id),
    value: varchar('value', { length: 255 }),
    comments: varchar('comments', { length: 255 }),
    update_medium_id: integer('update_medium_id').references(() => managedListItems.id),
...
export const dataEntryFeedbacks = pgTable('data_entry_feedbacks', {
    ...
    feedback_by_id: text('feedback_by_id').notNull().references(() => user.id),
...
export const dataEntryLogs = pgTable('data_entry_logs', {
    ...
    updated_by_id: text('updated_by_id').notNull().references(() => user.id),
