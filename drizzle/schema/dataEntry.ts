...
import { generators } from './utility';
...
export const inputDefinitions = pgTable('input_definitions', {
    ...
    category_id: uuid('category_id').notNull().references(() => managedListItems.id),
    subcategory_id: uuid('subcategory_id').notNull().references(() => managedListItems.id),
    energy_provider_id: uuid('energy_provider_id').references(() => managedListItems.id),
    energy_type_id: uuid('energy_type_id').references(() => managedListItems.id),
    energy_source_id: uuid('energy_source_id').references(() => managedListItems.id),
    unit_id: uuid('unit_id').notNull().references(() => managedListItems.id),
    data_type_id: uuid('data_type_id').notNull().references(() => managedListItems.id),
    ...
    agg_level_id: uuid('agg_level_id').references(() => managedListItems.id),
...
export const dataEntries = pgTable('data_entries', {
    ...
    generator_id: uuid('generator_id').references(() => generators.id),
    input_def_id: uuid('input_def_id').notNull().references(() => inputDefinitions.id),
    value: varchar('value', { length: 255 }).notNull(),
    comments: varchar('comments', { length: 255 }),
    update_medium_id: uuid('update_medium_id').references(() => managedListItems.id),
    status_id: uuid('status_id').references(() => managedListItems.id),
...
export const dataEntryFeedbacks = pgTable('data_entry_feedbacks', {
    ...
    feedback_by_id: uuid('feedback_by_id').notNull().references(() => user.id),
    feedback_date: uuid('feedback_date').notNull(),
    reply: varchar('reply', { length: 255 }),
    reply_by_id: uuid('reply_by_id').references(() => user.id),
...
export const dataEntryLogs = pgTable('data_entry_logs', {
    ...
    updated_by_id: uuid('updated_by_id').notNull().references(() => user.id),
