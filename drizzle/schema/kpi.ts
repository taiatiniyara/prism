import { boolean, integer, json, pgTable, serial, uuid, varchar } from "drizzle-orm/pg-core";
...
export const kpiDefinitions = pgTable("kpi_definitions", {
    ...
    type_id: integer("type_id").notNull().references(() => managedListItems.id),
});
...
export const kpi = pgTable("kpi", {
    ...
    report_period_id: serial("report_period_id").notNull().references(() => reportPeriods.id),
    kpi_def_id: serial("kpi_def_id").notNull().references(() => kpiDefinitions.id),
...
export const bsc = pgTable("bsc", {
    id: uuid("id").primaryKey().notNull().defaultRandom(),
    kpi_id: uuid("kpi_id").notNull().references(() => kpi.id),
    perspective_level: integer("perspective_level").notNull().$type<PerspectiveLevel>(),
