...
export async function GET(req: Request) {
  ...
  const allSa = await db.select().from(serviceAreas).where(and(eq(serviceAreas.is_active, true), eq(serviceAreas.is_virtual, false)));
      ...
      Data: allSa.filter((sa) => sa.utility_id === urp.utility_id).map((sa) => {
        const dataValues = inputDefs.reduce((acc, dl) => {
          const val = entries.find((l) =>
            ...
            allResources.find((g) => g.id === l.energy_resource_id)?.service_area_id === sa.id
          )?.value;
