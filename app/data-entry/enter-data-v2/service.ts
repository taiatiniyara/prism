...
export async function getMeasureEntryFilterViewModel(): Promise<MeasureEntryPageViewModel> {
  ...
  const [
    ...
    getManagedListItems("Energy Provider"),
    getManagedListItems("Energy Type"),
    getManagedListItems("Energy Source"),
  ...
  const rawRows = await db
    .select({
      ...
      valueBoolean: dataEntries.boolean_value,
      valueOptionId: dataEntries.value_option_id,
      valueString: dataEntries.text_value,
      energyProviderId: dataEntries.energy_provider_id,
      energyTypeId: dataEntries.energy_type_id,
  ...
  const viewRows: MeasureEntryRowView[] = rawRows.map((r) => {
    ...
    return {
      ...
      energyResourceId: r.energyResourceId ?? null,
  ...
  const dimChecks: [string, number[]][] = [
    ["energy_provider", viewRows.map((r) => r.energyProviderId)],
    ["energy_type", viewRows.map((r) => r.energyTypeId)],
    ["energy_source", viewRows.map((r) => r.energySourceId)],
