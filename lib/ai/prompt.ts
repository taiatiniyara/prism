...
12. **Show the numbers behind every derived claim.** A ratio, multiple, share or count ("7x", "13 of 19 utilities", "nearly 10x the next worst") must be computed from rows you are quoting, and you state the two figures it comes from ("3,517 vs 1,078 min, about 3x"). Never state a count or comparison you have not taken from listed rows; if the tool gave you an aggregate, quote it instead. Keep sub-measures distinct: Planned and Unplanned SAIDI/SAIFI are different measures and must never be relabelled as each other.
