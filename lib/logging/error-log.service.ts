  ...
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width:640px;margin:0 auto;border-collapse:separate;border-spacing:0;background:#fff;border:1px solid #dce3ef;border-radius:14px;overflow:hidden">
    ...
    <tr>
      <td style="padding:0;background:#450a0a">
        ...
        <div style="padding:0 24px 20px 24px;color:#fff;font-size:24px;line-height:1.25;font-weight:700">${escapeHtml(truncate(input.message, 120))}</div>
      </td>
    </tr>
    <tr>
      <td style="padding:18px 24px 8px 24px">
        <div style="display:inline-block;padding:5px 10px;border-radius:999px;background:${severityBadge};color:#fff;font-size:11px;text-transform:uppercase;letter-spacing:.06em;font-weight:700">${escapeHtml(severityLabel)}</div>
    ...
    <tr>
      <td style="padding:8px 24px 12px 24px">
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="border:1px solid #e3e8f2;border-radius:10px;border-collapse:separate;border-spacing:0;overflow:hidden">
          <tr><td style="padding:8px 12px;border-bottom:1px solid #e3e8f2;width:160px;color:#4f5d75;font-size:13px;font-weight:600;vertical-align:top">Source</td><td style="padding:8px 12px;border-bottom:1px solid #e3e8f2;color:#17213a;font-size:13px;line-height:1.5;vertical-align:top">${escapeHtml(input.source)}</td></tr>
          <tr><td style="padding:8px 12px;border-bottom:1px solid #e3e8f2;color:#4f5d75;font-size:13px;font-weight:600;vertical-align:top">Type</td><td style="padding:8px 12px;border-bottom:1px solid #e3e8f2;color:#17213a;font-size:13px;line-height:1.5;vertical-align:top">${escapeHtml(input.errorType)}</td></tr>
          ${input.url ? `<tr><td style="padding:8px 12px;border-bottom:1px solid #e3e8f2;color:#4f5d75;font-size:13px;font-weight:600;vertical-align:top">URL</td><td style="padding:8px 12px;border-bottom:1px solid #e3e8f2;color:#17213a;font-size:13px;line-height:1.5;vertical-align:top">${escapeHtml(input.url)}</td></tr>` : ""}
          ${input.userEmail ? `<tr><td style="padding:8px 12px;border-bottom:1px solid #e3e8f2;color:#4f5d75;font-size:13px;font-weight:600;vertical-align:top">User</td><td style="padding:8px 12px;border-bottom:1px solid #e3e8f2;color:#17213a;font-size:13px;line-height:1.5;vertical-align:top">${escapeHtml(input.userEmail)}${input.userRole ? ` (${escapeHtml(input.userRole)})` : ""}</td></tr>` : ""}
          <tr><td style="padding:8px 12px;color:#4f5d75;font-size:13px;font-weight:600;vertical-align:top">Message</td><td style="padding:8px 12px;color:#17213a;font-size:13px;line-height:1.5;vertical-align:top;word-break:break-word">${escapeHtml(truncate(input.message, 500))}</td></tr>
        </table>
      </td>
    </tr>
    ${input.stack ? `<tr><td style="padding:8px 24px 12px 24px"><div style="padding:12px 14px;border-radius:10px;background:#fef2f2;border:1px solid #fecaca;color:#991b1b;font-size:11px;line-height:1.5;font-family:monospace;max-height:200px;overflow-y:auto;white-space:pre-wrap;word-break:break-all">${escapeHtml(truncate(input.stack, 2000))}</div></td></tr>` : ""}
    <tr>
