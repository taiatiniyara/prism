...
const EXTRACT_COLUMNS = {
  ...
  provider: ["energy_provider_id", "provider_id", "provider"],
  type: ["energy_type_id", "type_id", "type"],
  source: ["energy_source_id", "source_id", "source"],
  resource_type: ["energy_resource_type_id", "resource_type_id", "resource_type"],
  ...
  energyResourceId: ["energy_resource_id", "energy_resource"],
...
export async function parseExtractWorkbook(
  ...
  ws.eachRow((r, rowNumber) => {
    ...
    rows.push({
      ...
      energyResourceId: toInt(get(r, "energyResourceId")),
