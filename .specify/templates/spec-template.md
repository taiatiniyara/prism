...
- **CA-001 Security**: Specify authentication and authorization expectations for
  each protected flow, endpoint, or mutation introduced by this feature.
- **CA-002 Data Integrity**: Describe schema/data mutation impact, including
  affected entities, reporting implications, and required Drizzle update path.
- **CA-003 Validation**: Define mandatory validation evidence for delivery
  (`npm run lint`, `npm run build`, and automated tests when behavior changes).
- **CA-004 UX Accessibility**: Define loading/empty/error states and
  keyboard/screen-reader expectations for all new or changed user interactions.
- **CA-005 UI Standards**: Specify how UI work will use Tailwind CSS and
  shadcn-compatible component primitives.
- **CA-006 Reuse**: Identify repeated UI or logic patterns and define reusable
  component/service extraction requirements.
